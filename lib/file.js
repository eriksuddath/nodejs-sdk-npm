 ...
 */
  ...
  */
		...
		value: function _validate(options) {
			...
			var validate = {
				...
				versionTag: function versionTag() {
					return (0, _const.V_versionTag)(options.versionTag);
				}
			};
  ...
  */
		...
		key: 'upload',
		value: function upload(filesUpload, versionTag) {
			var _this = this;

			/*
   	ALPHA ROUTE: NEED TO UPDATE TO APIGEE
   */
			this._validate({ filesUpload: filesUpload, versionTag: versionTag });
			...
			return _promise2.default.all(allOptions.map(function (options) {
				...
				return _requestPromise2.default.post(options).then(_this._transformResponse).then(function (_ref) {
					...
					return files_ids.map(function (fileId, index) {
						...
						return (0, _assign2.default)({}, { fileId: fileId, filename: filename, versionTag: versionTag });
					});
				});
			});
		}
  ...
  */
		...
		value: function json(languageId, milestoneId, fileIds) {
			...
			var custom = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : { offset: 0, limit: 100 };
